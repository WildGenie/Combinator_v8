  ...
  if (inputComplete) {
    ...
    switch (makeWord(inputArray[0],inputArray[1])) {
     ...
     case fnTIMING_SEQUENCE:
        ...
        // Check the division factor
        if (divisionFactor == 1 | divisionFactor == 2 | divisionFactor == 4 | divisionFactor == 6 | divisionFactor == 10 | divisionFactor == 20)
             {}
        else {break;}
        ...
        for (word j=0;j<500;j++)
        {
           if ((j == (lampDivide/divisionFactor))) {
                 // Wait for pin LOW
                 while (PIND & (1<<2)) {}
            ...
            } else if ((j % (lampDivide/divisionFactor)) == 0) {
                 // Wait for pin LOW
                 while (PIND & (1<<2)) {}
                 PORTB = 0;
                 // Wait for pin HIGH
           ...
           for (int i = 0; i < (divisionFactor - 1); i++) {
                 // Wait for pin LOW
                 while (PIND & (1<<2)) {}
