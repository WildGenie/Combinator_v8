...
classdef PropertyGrid < PropertyGridPackage.UIControl
    ...
    properties (Access = private)
        ...
        Model;
        Fields = PropertyGridPackage.JidePropertyGridField.empty(1,0);
        % The MatLab structure or object bound to the property grid.
        BoundItem = [];
    end
    methods
        function self = PropertyGrid(varargin)
            self = self@PropertyGridPackage.UIControl(varargin{:});
        end
        ...
        function self = Instantiate(self, parent)
            ...
            com.jidesoft.grid.CellEditorManager.registerEditor(PropertyGridPackage.javaclass('cellstr',1), com.jidesoft.grid.StringArrayCellEditor);
            com.jidesoft.grid.CellEditorManager.registerEditor(PropertyGridPackage.javaclass('char',1), com.jidesoft.grid.MultilineStringCellEditor, com.jidesoft.grid.MultilineStringCellEditor.CONTEXT);
            com.jidesoft.grid.CellRendererManager.registerRenderer(PropertyGridPackage.javaclass('char',1), com.jidesoft.grid.MultilineStringCellRenderer, com.jidesoft.grid.MultilineStringCellEditor.CONTEXT);

            % create JIDE property pane
            self.Table = handle(PropertyGridPackage.objectEDT('com.jidesoft.grid.PropertyTable'), 'CallbackProperties');  % property grid (without table model)
            self.Pane = PropertyGridPackage.objectEDT('com.jidesoft.grid.PropertyPane', self.Table);  % property pane (with icons at top and help panel at bottom)
            ...
            set(self.Table, 'KeyPressedCallback', @PropertyGridPackage.PropertyGrid.OnKeyPressed);
        end
        ...
        % Setting this property clears any object bindings.
            import PropertyGridPackage.*
            validateattributes(properties, {'PropertyGridField'}, {'vector'});
            
            % Remove deleted properties
            %properties = self.Properties(isvalid(self.Properties));
            ...
            self.Fields = PropertyGridPackage.JidePropertyGridField.empty(0,1);
            for k = 1 : numel(properties)
                self.Fields(k) = PropertyGridPackage.JidePropertyGridField(properties(k));
            end
            ...
            set(model, 'PropertyChangeCallback', @PropertyGridPackage.PropertyGrid.OnPropertyChange);
        end
    ...
    methods (Access = private)
        ...
        % Updates a property value or reverts changes if value is illegal.
            ...
            value = field.Value;
            callback_function = field.PropertyData.ValueChangeCallback;
            ...
            if field.CanAccept(value)
                try
                    ...
                    % Fire the appropriate callback function
                    callback_function(name,field.PropertyData.Value,value);
                catch me
    ...
    methods (Access = private, Static)
        ...
        %    a com.jidesoft.grid.DefaultProperty instance
            ...
            h = PropertyGridPackage.findobjuser(@(userdata) userdata.(member) == obj, '__PropertyGrid__');
            self = get(h, 'UserData');
        end
    ...
    methods (Static)  % methods (Access = private, Static) for MatLab 2010a and up
        ...
        % could be feasible, it leads to a memory leak.
            self = PropertyGridPackage.PropertyGrid.FindPropertyGrid(obj, 'Model');
            name = get(event, 'PropertyName');  % JIDE automatically uses a hierarchical naming scheme
            self.UpdateField(name);

            if 0  % debug mode
                oldvalue = PropertyGridPackage.var2str(get(event, 'OldValue'));  % old value as a string
                newvalue = PropertyGridPackage.var2str(get(event, 'NewValue'));  % new value as a string
