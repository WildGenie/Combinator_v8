...
classdef JidePropertyGridField < handle
    ...
    properties (Access = private)
        ...
        Children = PropertyGridPackage.JidePropertyGridField.empty(0,1);
    ...
    methods
        ...
        % May register editor contexts with the global CellEditorManager.
            validateattributes(self, {'PropertyGridPackage.JidePropertyGridField'}, {'scalar'});
                ...
                case 'scalar'
                    switch data.Type.PrimitiveType
                        ...
                        case {'densecomplexdouble','sparsecomplexdouble','densecomplexsingle','sparsecomplexsingle'}
                            matlabtype = [];
                            field.setType(PropertyGridPackage.javaclass('char',1));  % edit as string and convert with eval
                        otherwise
                            matlabtype = [];
                    ...
                    if ~isempty(matlabtype)
                        javatype = PropertyGridPackage.javaclass(matlabtype);
        ...
        % Finds the property field with the given name in an array.
            self = selfarray.FindByNameRecurse(PropertyGridPackage.strsplit(name, '.'));
        end
        ...
        function list = GetTableModel(selfarray)
            ...
            list = PropertyGridPackage.javaArrayList(ctrls);
        end
    end
    methods (Access = private)
        function self = FindByNameRecurse(selfarray, nameparts)
            names = PropertyGridPackage.getclassfield(PropertyGridPackage.getclassfield(selfarray, 'PropertyData'), 'Name');
            ix = strmatch(nameparts{1}, names, 'exact');
            if isempty(ix)
                self = PropertyGridPackage.JidePropertyGridField.empty(0,1);
                return;
            end
            ix = ix(1);
        ...
        function InitializeVector(self, field)
            ...
            switch data.Type.PrimitiveType
                case 'char'  % add a string property
                    switch data.Type.Shape
                        case 'row'
                            field.setType(PropertyGridPackage.javaclass('char',1));
                            if ~isempty(data.Type.Domain)
                                self.AddComboBoxEditor(field, PropertyGridPackage.javaclass('char',1), PropertyGridPackage.javaStringArray(data.Type.Domain));
                            end
                        otherwise
                            field.setType(PropertyGridPackage.javaclass('char',1));  % edit as string and convert with eval
                    end
                ...
                case 'logical'
                    if ~isempty(data.Type.Domain)
                        field.setType(PropertyGridPackage.javaclass('cellstr',1));  % java.lang.String array
        ...
        %    a cell array of strings to label elements in the set shown
            editor = com.jidesoft.grid.CheckBoxListComboBoxCellEditor(javaStringArray(labels), PropertyGridPackage.javaclass('cellstr',1));
            self.ApplyContext(field, PropertyGridPackage.javaclass('cellstr',1), editor, 'checkboxlist');
        end
